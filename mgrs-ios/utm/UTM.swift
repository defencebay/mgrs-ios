...
public class UTM {
    ...
    public func toPoint() -> GridPoint {
        ...
        let ep2: Double = 0.006739496742
        let k0: Double = 0.9996
        let rM: Double = 6366197.724
        let rE: Double = 6399593.625
        let e0: Double = easting - 500000.0

        // Footpoint latitude
        let phi1: Double = north / rM / k0
        let cosPhi1: Double = cos(phi1)
        let sinPhi1: Double = sin(phi1)
        let sin2Phi1: Double = sin(2.0 * phi1)
        let cosPhi1Sq: Double = cosPhi1 * cosPhi1

        // Radius of curvature in prime vertical
        let N1: Double = k0 * rE / sqrt(1.0 + ep2 * cosPhi1Sq)
        let T1: Double = e0 / N1

        // Meridional arc series
        let c1: Double = ep2 * 3.0 / 4.0
        let c1Sq: Double = c1 * c1
        let c1Cu: Double = c1Sq * c1
        let s1: Double = phi1 + sin2Phi1 / 2.0
        let s2: Double = 3.0 * s1 + sin2Phi1 * cosPhi1Sq
        let s3: Double = 5.0 * s2 / 4.0 + sin2Phi1 * cosPhi1Sq * cosPhi1Sq
        let M: Double = k0 * rE * (phi1 - c1 * s1 + c1Sq * 5.0 / 3.0 * s2 / 4.0 - c1Cu * 35.0 / 27.0 * s3 / 3.0)

        // Correction terms
        let T1SqCorr: Double = ep2 * T1 * T1 / 2.0 * cosPhi1Sq
        let expArg: Double = T1 * (1.0 - T1SqCorr / 3.0)
        let sinhTerm: Double = (exp(expArg) - exp(-expArg)) / 2.0
        let corrArg: Double = (north - M) / N1 * (1.0 - T1SqCorr) + phi1

        // Atan chain
        let innerAtan: Double = atan(sinhTerm / cos(corrArg))
        let outerAtan: Double = atan(cos(innerAtan) * tan(corrArg))
        let latCorr: Double = outerAtan - phi1

        // Latitude
        var latitude: Double = (phi1 + (1.0 + ep2 * cosPhi1Sq - ep2 * sinPhi1 * cosPhi1 * latCorr * 3.0 / 2.0) * latCorr) * 180.0 / Double.pi
